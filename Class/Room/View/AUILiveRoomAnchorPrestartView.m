...
- (instancetype)initWithFrame:(CGRect)frame withModel:(nonnull AUIInteractionLiveInfoModel *)model {
    ...
    if (self) {
        ...
        if (model.cover.length > 0) {
            [cover yy_setImageWithURL:[NSURL URLWithString:model.cover] placeholder:AUIInteractionLiveGetCommonImage(@"ic_list_bg")];
        }
        else {
            cover.image = AUIInteractionLiveGetCommonImage(@"ic_list_bg");
        }
        ...
        [startLiveButton setBackgroundColor:AUIInteractionLiveColourfulFillStrong forState:UIControlStateNormal];
        ...
        beautyButton.image = AUIInteractionLiveGetCommonImage(@"ic_living_beauty");
        ...
        switchCameraButton.image = AUIInteractionLiveGetCommonImage(@"ic_living_camera");
