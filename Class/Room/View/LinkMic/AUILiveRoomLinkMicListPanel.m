...
@property (nonatomic, copy, readonly) AUIInteractionLiveUser *joinUser;
...
- (void)setData:(id)data {
    ...
    if (self.itemType == AUILiveRoomLinkMicItemTypeApply) {
        _joinUser = (AUIInteractionLiveUser *)self.data;
    }
    else {
        _joinUser = [AUIInteractionLiveUser new];
        _joinUser.userId = [(AUILiveRoomRtcPull *)self.data joinInfo].userId;
        _joinUser.nickName = [(AUILiveRoomRtcPull *)self.data joinInfo].userNick;
        _joinUser.avatar = [(AUILiveRoomRtcPull *)self.data joinInfo].userAvatar;
    }
}

- (BOOL)videoOff {
    if (self.itemType == AUILiveRoomLinkMicItemTypeJoined) {
        AUIInteractionLiveLinkMicJoinInfoModel *joinInfo = [(AUILiveRoomRtcPull *)self.data joinInfo];
...
- (instancetype)initWithFrame:(CGRect)frame {
    ...
    if (self) {
        ...
        [_agreeBtn setBackgroundColor:AUIInteractionLiveColourfulFillStrong forState:UIControlStateNormal];
        ...
        [_micBtn setImage:AUIInteractionLiveGetImage(@"ic_living_linkmic_audio") forState:UIControlStateNormal];
        [_micBtn setImage:AUIInteractionLiveGetImage(@"ic_living_linkmic_audio_selected") forState:UIControlStateSelected];
        ...
        [_cameraBtn setImage:AUIInteractionLiveGetImage(@"ic_living_linkmic_camera") forState:UIControlStateNormal];
        [_cameraBtn setImage:AUIInteractionLiveGetImage(@"ic_living_linkmic_camera_selected") forState:UIControlStateSelected];
...
- (instancetype)initWithFrame:(CGRect)frame withManager:(AUILiveRoomLinkMicManagerAnchor *)manager {
    ...
    if (self) {
        ...
        selectedLine.backgroundColor = AUIInteractionLiveColourfulFillStrong;
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    cell.onLeaveBtnClick = ^(AVBlockButton *sender, AUILiveRoomLinkMicCellItem *cellItem) {
        [weakSelf.manager kickoutLinkMic:[(AUILiveRoomRtcPull *)cellItem.data joinInfo].userId completed:^(BOOL success) {
            if (success) {
                [weakSelf reload];
            }
            else {
    ...
    cell.onMicBtnClick = ^(AVBlockButton *sender, AUILiveRoomLinkMicCellItem *cellItem) {
        AUIInteractionLiveLinkMicJoinInfoModel *joinInfo = [(AUILiveRoomRtcPull *)cellItem.data joinInfo];
        [weakSelf.manager openMic:joinInfo.userId needOpen:!joinInfo.micOpened completed:^(BOOL success) {
            if (success) {
                joinInfo.micOpened = !joinInfo.micOpened;
                [weakSelf reload];
            }
            else {
                [AVToastView show:@"失败了，请稍后重试" view:weakSelf.manager.roomVC.view position:AVToastViewPositionMid];
    ...
    cell.onCameraBtnClick = ^(AVBlockButton *sender, AUILiveRoomLinkMicCellItem *cellItem) {
        ...
        [weakSelf.manager openCamera:joinInfo.userId needOpen:!joinInfo.cameraOpened completed:^(BOOL success) {
            if (success) {
                joinInfo.cameraOpened = !joinInfo.cameraOpened;
                [weakSelf reload];
            }
            else {
                [AVToastView show:@"失败了，请稍后重试" view:weakSelf.manager.roomVC.view position:AVToastViewPositionMid];
...
//            AUILiveRoomLinkMicCellItem *cellItem = [AUILiveRoomLinkMicCellItem new];
//            cellItem.itemType = AUILiveRoomLinkMicItemTypeApply;
//            cellItem.data = AUIInteractionAccountManager.me;
//            [self.list addObject:cellItem];
//        }
        ...
        // for test
//        for (NSUInteger i=0; i<0; i++) {
//            AUILiveRoomRtcPull *pull = [AUILiveRoomRtcPull new];
//            AUIInteractionLiveLinkMicJoinInfoModel *joinInfo = [[AUIInteractionLiveLinkMicJoinInfoModel alloc] init:AUIInteractionAccountManager.me.userId userNick:@"哈哈水水水水水水水水哒哒哒哒哒哒多多哒哒哒哒哒哒多" userAvatar:AUIInteractionAccountManager.me.avatar rtcPullUrl:@""];
