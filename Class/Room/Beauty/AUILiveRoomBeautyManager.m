...
@interface AUILiveRoomBeautyManager ()<QueenMaterialDelegate>
...
@implementation AUILiveRoomBeautyManager

+ (instancetype)manager {
    static AUILiveRoomBeautyManager *manager = nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        manager = [[AUILiveRoomBeautyManager alloc] init];
    });
    return manager;
}
...
+ (void)registerBeautyEngine {
    ...
    [AUILiveRoomBeautyController setupMotionManager];
}

+ (void)checkResourceWithCurrentView:(UIView *)view completed:(void (^)(BOOL completed))completed {
    [AUILiveRoomBeautyManager manager].checkResult = completed;
    [[AUILiveRoomBeautyManager manager] startCheckWithCurrentView:view];
}
